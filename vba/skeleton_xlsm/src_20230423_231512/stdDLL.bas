...
#ElseIf VBA7 And Win64 Then
    #If IS_DEBUG Then
        Private Const DLL_DIRECTORY As String = "C:\Users\tani\Dropbox\workspace\github\utils\vba\bin\x64\Debug"
    #Else
        Private Const DLL_DIRECTORY As String = "C:\Users\tani\Dropbox\workspace\github\utils\vba\bin\x64\Release"
    #End If
#Else
    #If IS_DEBUG Then
        Private Const DLL_DIRECTORY As String = "C:\Users\tani\Dropbox\workspace\cpp\QuantLibCode\QuantLibDLL\bin\Win32\Debug"
    #Else
        Private Const DLL_DIRECTORY As String = "C:\Users\tani\Dropbox\workspace\github\utils\vba\bin\Win32\Release"
    #End If
#End If
...
#Else
    ...
    Private Declare Sub QLDLL_createIRSScheduleByTerminationPeriod Lib "QuantLibDLL.dll" (ByRef fixedStartDates As Long, ByRef fixedEndDates As Long, ByRef fixedFixingDates As Long, ByRef fixedPaymentDates As Long, ByRef floatingStartDates As Long, ByRef floatingEndDates As Long, ByRef floatingFixingDates As Long, ByRef floatingPaymentDates As Long, ByVal outputSize As Long, ByVal effectiveDate As Long, ByVal terminationPeriod As String, ByVal fixedRate As Double, ByVal tenor As String, ByVal calendar As String, ByVal delim As String, ByVal convention As String, ByVal terminationDateConvention As String, ByVal dateGeneration As String, ByVal endOfMonth As Boolean, ByVal firstDate As Long, ByVal nextToLastDate As Long)
...
Public Sub createIRSScheduleByTerminationPeriod( _
    ByRef outputFixedStartDates() As Date, ByRef outputFixedEndDates() As Date, ByRef outputFixedFixingDates() As Date, ByRef outputFixedPaymentDates() As Date, _
    ByRef outputFloatingStartDates() As Date, ByRef outputFloatingEndDates() As Date, ByRef outputFloatingFixingDates() As Date, ByRef outputFloatingPaymentDates() As Date, _
    ByVal effectiveDate As Date, ByVal terminationPeriod As String, _
    ByVal tenor As String, ByVal calendar As String, _
    Optional ByVal delim As String = "+", _
    Optional ByVal convention As String = "MODIFIEDFOLLOWING", _
    Optional ByVal terminationDateConvention As String = "MODIFIEDFOLLOWING", _
    ...
    ' fixed dates setting
    Dim fixedStartDates() As Long
    Dim fixedEndDates() As Long
    Dim fixedFixingDates() As Long
    Dim fixedPaymentDates() As Long
    ReDim fixedStartDates(0 To buffSize) As Long
    ReDim fixedEndDates(0 To buffSize) As Long
    ReDim fixedFixingDates(0 To buffSize) As Long
    ReDim fixedPaymentDates(0 To buffSize) As Long
    
    ' floating dates setting
    Dim floatingStartDates() As Long
    Dim floatingEndDates() As Long
    Dim floatingFixingDates() As Long
    Dim floatingPaymentDates() As Long
    ReDim floatingStartDates(0 To buffSize) As Long
    ReDim floatingEndDates(0 To buffSize) As Long
    ReDim floatingFixingDates(0 To buffSize) As Long
    ReDim floatingPaymentDates(0 To buffSize) As Long
    
    Call QLDLL_createIRSScheduleByTerminationPeriod( _
        fixedStartDates(LBound(fixedStartDates)), fixedEndDates(LBound(fixedEndDates)), fixedFixingDates(LBound(fixedFixingDates)), fixedPaymentDates(LBound(fixedPaymentDates)), _
        floatingStartDates(LBound(floatingStartDates)), floatingEndDates(LBound(floatingEndDates)), floatingFixingDates(LBound(floatingFixingDates)), floatingPaymentDates(LBound(floatingPaymentDates)), buffSize, _
        CLng(effectiveDate), terminationPeriod, _
        0.05, tenor, calendar, delim, convention, terminationDateConvention, _
        dateGeneration, endOfMonth, _
        IIf(firstDate = NULL_DATE, 0, CLng(firstDate)), _
        IIf(nextToLastDate = NULL_DATE, 0, CLng(nextToLastDate)))
    
    Call createOutputDateArray(fixedStartDates, outputFixedStartDates, buffSize)
    Call createOutputDateArray(fixedEndDates, outputFixedEndDates, buffSize)
    Call createOutputDateArray(fixedFixingDates, outputFixedFixingDates, buffSize)
    ' Call createOutputDateArray(fixedPaymentDates, outputFixedPaymentDates, buffSize)
    
    Call createOutputDateArray(floatingStartDates, outputFloatingStartDates, buffSize)
    Call createOutputDateArray(floatingEndDates, outputFloatingEndDates, buffSize)
    Call createOutputDateArray(floatingFixingDates, outputFloatingFixingDates, buffSize)
    ' Call createOutputDateArray(floatingPaymentDates, outputFloatingPaymentDates, buffSize)
End Sub

Private Sub createOutputDateArray(ByRef rawArray() As Long, ByRef outputArray() As Date, Optional ByVal buffSize As Long = 1000)
    Dim i As Long
    ReDim outputArray(0 To buffSize) As Date
    For i = 0 To buffSize
        If rawArray(i) = 0 Then
            Exit For
        Else
            outputArray(i) = CDate(rawArray(i))
        End If
    Next i
    ReDim Preserve outputArray(0 To i - 1) As Date
End Sub
