...
Public Sub buildSetterGetter(ByVal memberName As String, ByVal memberTypeName As String, _
    ...
    Debug.Print paste(vbNullString, "Public Property Get get_", memberName, "() As ", convertedMemberTypeName, ": get_", memberName, " = ", memberName, ": End Property")
End Sub
...
Public Function replaceString( _
    ...
    With reg
        ...
        .ignoreCase = ignoreCase
        .pattern = pattern
        replaceString = .Replace(fromString, toString)
    End With
    Set reg = Nothing
End Function
...
Public Function isLeapYear(ByVal targetDate As Date) As Boolean
    isLeapYear = (Day(DateSerial(Year(targetDate), 3, 1) - 1) = 29)
End Function

Public Function unixtime2serial(ByVal unixtime As Double, Optional ByVal isUTC As Boolean = True) As Double
    unixtime2serial = (unixtime + 32400 * IIf(isUTC, 1, 0)) / 86400 + 25569
End Function

Public Function convertTermStr(ByVal termStr As String, Optional ByVal freq As String = "M") As String
    Dim tmpStr As String
    Dim outputStr As String
    Dim freqList As Variant
    Dim buff As Variant
    
    tmpStr = UCase(termStr)
    freqStr = Array("D", "W", "M", "Y")
    
    For Each iter In freqStr
        buff = Split(tmpStr, buff)
        
        If IsNumeric(buff(0)) Then
            Select Case freqStr
                Case "D"
    ...
    convertTermStr = outputStr
End Function

Private Function convertFreq(ByVal term As Double, ByVal fromFreq As String, ByVal toFreq As String) As Double
    Select Case Trim(UCase(fromFreq))
        Case "D"
            Select Case Trim(UCase(toFreq))
                Case "D"
                    convertFreq = term
                Case "W"
                    convertFreq = term * 4
